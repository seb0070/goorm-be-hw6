...
- 과제 문서에 명시된 요구사항(상품 조회 → 로그인 → 장바구니 → 총액)만 기준으로 설계함
- 다르게 설계한 UI·기능: cart는 `productId`/`quantity`만 저장하고 이름·가격은 selector에서 상품 카탈로그와 조인(중복 저장 없음), 로그인 실패를 세션 상태(`status`)와 분리된 `error` 필드로 관리해 실패해도 화면이 깨지지 않게 함, 로그아웃 시 장바구니를 자동으로 비움
- 복제하지 않은 이미지·브랜드·문구: "ShopMall"은 임의로 지은 이름(실존 브랜드 아님), Google 로그인 버튼은 Firebase 기본 SDK 동작을 그대로 사용(별도 로고 이미지 제작 안 함), 상품 이미지는 Fake Store API가 제공하는 원본 URL을 그대로 참조(직접 제작한 브랜드 자산 없음)

## 2. 실행 화면

![스크린샷 1](./screenshots/image1.png)

![스크린샷 2](./screenshots/image2.png)
...
| 장바구니 담기·목록              | ☑ 완료                                                 | 상품 목록에서 "장바구니 담기" → 토스트/버튼 피드백 확인 → `/cart` 확인 | `cartSlice.ts`, `CartPage.tsx`, `ProductListPage.tsx`           |
| 총액 계산                       | ☑ 완료                                                 | 여러 상품 담고 총액 비교                                               | `selectCartTotal`                                               |
| Firebase 로그인                 | ☑ 완료                                                 | `/login`에서 Google 로그인                                             | `googleAuth.ts`                                                 |
| 인증 초기·사용자 상태           | ☑ 완료 (초기 로딩 시 화면 표시는 미흡, 아래 한계 참고) | 새로고침 시 헤더 프로필 아이콘 변화 관찰                               | `authSlice.ts`, `authListener.ts`, `Header.tsx`                 |
| 로그인 오류·로그아웃            | ☑ 완료                                                 | 로그인 팝업 닫아서 에러 확인, 프로필 드롭다운에서 로그아웃 클릭        | `LoginPage.tsx`, `Header.tsx`                                   |
...
| 로그인 전후 UI   | ☑ 완료   | 헤더 우측이 "로그인 아이콘" ↔ "프로필 아바타(클릭 시 이름+로그아웃 드롭다운)"으로 전환 |
...
| 수량 배지     | ☑ 적용        | 헤더 장바구니 아이콘에 `selectCartItemCount` 기반 배지 연결 (`Header.tsx`)                                                                                                                  |
| 반응형·접근성 | ☑ 적용 (부분) | 이미지 `alt`, 에러 `role="alert"/"status"`, 모바일 브레이크포인트(`product-grid`/`cart-layout`/`app-nav`) CSS 추가. 프로필 드롭다운은 마우스 바깥 클릭으로만 닫히고 키보드(Esc) 대응은 없음 |
...
| action   | `resetCart`           | 장바구니를 완전히 비움(`items: []`). `authListener`가 로그인→로그아웃 전환 시에만 dispatch | Google 로그인 후 담기 → 로그아웃 → `/cart` 빈 상태 확인        |
| selector | `selectCartItemCount` | 전체 수량 합. 헤더 장바구니 아이콘 배지에 연결                                             | 상품 여러 개 담고 헤더 배지 숫자와 실제 합 비교                |
...
| 로그아웃 시 cart | 초기화됨 — `cartSlice` 자체는 여전히 `auth`를 모르지만, `authListener.ts`가 로그인→로그아웃 실제 전환 시에만 `resetCart`를 dispatch (게스트로 새로고침할 때는 발동 안 함) |
...
- 로그인 성공 화면: 헤더 우측이 로그인 아이콘에서 프로필 아바타(Google 프로필 사진, 없으면 기본 사람 아이콘)로 전환, 클릭 시 이름/이메일 + 로그아웃 드롭다운 표시 (`Header.tsx`)
- 로그인 실패 안내: `/login`에 "Google 로그인에 실패했습니다. 다시 시도해 주세요." 문구 표시 (`role="alert"`)
- 인증 초기 로딩: `status: 'loading'` 상태로 시작, `onAuthStateChanged` 최초 콜백 전까지 유지
...
| photoURL    |  ✅  | ✅ (프로필 아바타 이미지, 없으면 기본 아이콘으로 대체) | 헤더에 본인에게만 표시, 저장소에 영속화 안 함(메모리만)                |
...
| 스타일      | 커스텀 CSS (`index.css`, 디자인 토큰 + 컴포넌트별 클래스) | —                | 카드형 그리드/장바구니 요약/프로필 드롭다운 등 전면 리디자인, 반응형 미디어쿼리 포함 |
| 아이콘      | lucide-react                                              | 1.27.0           | 장바구니/프로필/로그아웃 아이콘용 (Header.tsx)                                       |
| 언어        | TypeScript                                                | ~6.0.2           | 선택 기술로 채택, 전체 코드에 적용                                                   |
| AI 도구     | Claude Code (Sonnet 5)                                    | —                | 설계 상담, 코드 구현, 코드 리뷰, UI/UX 리디자인 전 과정에 사용                       |
...
- 패키지 관리자: npm 10.9.2
- 브라우저: 개발 중에는 `curl`로 HTTP 응답만 확인했고, 실제 화면 렌더링은 브라우저 자동화 도구가 없어 검증하지 못함 — Chrome 최신 버전 기준으로 직접 확인 필요
...
| 파일·폴더                              | 역할                                                    | 내가 수정한 내용                                                                                                                |
| -------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `src/app/store.ts`                     | Redux store 구성(`auth`/`cart`/`productsApi` 등록)      | AI가 초안대로 구현, 이후 직접 수정 없음                                                                                         |
| `src/features/products/`               | 상품 API 연동, 내부 타입 변환, mock, 목록 화면          | 필드명을 `image`가 아닌 `imageUrl`로 유지하도록 지시(요청안 대신 기존 명명 유지)                                                |
| `src/features/cart/`                   | 장바구니 상태(reducer)와 파생값(selector)               | 액션명을 `addToCart`가 아닌 `addItem`으로, `ProductCard` 별도 분리 없이 유지하도록 지시                                         |
| `src/features/auth/` + `src/firebase/` | Firebase Google 로그인/로그아웃, 인증 상태 동기화       | 로그인 방식을 이메일/비밀번호 대신 Google 단일 로그인으로 확정, 에러 처리는 `createAsyncThunk` 대신 컴포넌트 `try/catch`로 지시 |
| `src/routes/`                          | 라우트별 페이지(장바구니, 상품 상세는 아직 placeholder) | AI가 초안대로 구현, 이후 직접 수정 없음                                                                                         |
| `src/components/Header.tsx`            | 아이콘 기반 헤더(장바구니 배지, 프로필 아바타·드롭다운) | 루트(`/`)를 로그인 화면으로 바꾸는 옵션 대신 "로고 클릭 = 상품 목록" 방식으로 라우팅 구조를 유지하도록 지시                     |
...
→ ProductListPage
→ dispatch(addItem({ productId })) (+ 토스트·버튼 피드백)
→ 전역 cart state (cartSlice)
→ selectCartLineItems / selectCartTotal / selectCartItemCount
→ CartPage / Header 배지

Firebase Authentication (Google)
→ onAuthStateChanged 리스너 (authListener)
→ authSlice(status/user/error)
→ 초기(loading)·로그인(authenticated)·비로그인(unauthenticated)·오류 UI (Header)
→ 로그인→로그아웃 전환 시 authListener가 cart의 resetCart도 함께 dispatch
```
...
| ---: | --------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
|    1 | 요구사항·설계         | Claude Code | 공식 요구사항을 필수/권장/선택으로 구분하고, 1시간 착수~10일 제출 일정과 데이터·상태·인증·API·UI 작업 순서 요청                                       | 그대로 채택, 이후 작업 순서의 기준으로 사용                                                | 수정 없음(그대로 채택)                                                                                            |
|    2 | Redux 상태            | Claude Code | cart state/CartItem 구조, action·상태변경 규칙, 원본값·selector 파생값 구분, 중복상품·최소수량 정책을 먼저 설계로 확인 후 구현 요청                   | 설계안 그대로 구현                                                                         | 액션명 `addItem` 유지(제안 문서의 `addToCart` 대신), `ProductCard` 컴포넌트 분리 제안은 거절하고 인라인 구조 유지 |
|    3 | Firebase 인증         | Claude Code | Google 로그인 방식으로 설정·파일 역할, 인증 상태 흐름(loading/authenticated/unauthenticated/error 구분)을 먼저 설계로 확인 후 구현 요청               | 설계안 그대로 구현, 에러 처리는 `createAsyncThunk` 대신 컴포넌트 `try/catch` 방식 선택     | 로그인 방식을 이메일/비밀번호 대신 Google 단일 로그인으로 지정                                                    |
|    4 | 상품 API              | Claude Code | Fake Store API 응답 → 내부 product 매핑표, price는 number 유지, loading/error/empty, mock 대체, 이미지 실패 처리 설계 후 구현                         | 설계안 그대로 구현                                                                         | 필드명 `imageUrl` 유지(제안 문서의 `image` 대신)                                                                  |
|    5 | 통합 검토·오류        | Claude Code | 상품 API·전역 장바구니·Firebase 인증을 통합 검토(상태 소유 위치, 중복 상태, action/reducer/selector 책임, 보안/개인정보, 과도한 파일·라이브러리) 요청 | 발견된 문제(죽은 파일 4개) 삭제 반영, 경미한 auth 이슈 3건은 보류 결정                     | 경미한 인증 이슈 3건은 지금 고치지 말고 보류하라고 지시                                                           |
|    6 | UI/UX 리디자인        | Claude Code | 기존 Redux/Firebase 로직·파일 구조를 깨지 않는 조건으로 상품 그리드/장바구니/로그인 화면 전면 스타일링 요청                                           | CSS 전면 재작성 + 마크업에 클래스만 추가, 로직 변경 없음                                   | Google 브랜드 로고/아이콘 이미지는 추가하지 않고 텍스트 기반 버튼으로 유지하도록 지시                             |
|    7 | 장바구니·인증 UX 개선 | Claude Code | 담기 클릭 피드백(토스트·버튼 텍스트), 로그아웃 시 장바구니 초기화 요청                                                                                | `resetCart` 액션 추가 + `authListener`에서 로그인→로그아웃 전환 시에만 dispatch하도록 구현 | 수정 없음(제안대로 적용)                                                                                          |
|    8 | 헤더 아이콘화         | Claude Code | 텍스트 nav 링크를 아이콘(장바구니/프로필)으로 교체, 랜딩 라우트 유지 여부 확인 후 진행 요청                                                           | `Header.tsx` 분리, lucide-react 도입, `photoURL` 매핑 추가                                 | 루트를 로그인 화면으로 바꾸는 옵션 대신 "로고 클릭 = 상품 목록"(라우팅 구조 유지)으로 지정                        |
...
### 대표 프롬프트 3 (UI/UX 리디자인)

```text
현재 React 쇼핑몰 앱의 UI/UX 디자인을 현대적이고 깔끔한 쇼핑몰 스타일로 개선해줘.
목표:
1. Header/Navigation: 상단 고정 네비게이션 바
2. ProductListPage: 그리드 카드 레이아웃, 이미지 object-fit, 호버 효과,
   가격·카테고리 태그·담기 버튼 스타일링, 로딩 스피너/에러 배너
3. CartPage: 목록 카드 UI, 수량 +/- 버튼·삭제 버튼, 총액 요약 박스
4. LoginPage: 중앙 정렬 카드 UI
조건:
- 기존 Redux action, selector, Firebase auth 로직 및 파일 구조는 절대 깨뜨리지 말 것
- 반응형(모바일/데스크톱) 고려
```

### 대표 프롬프트 4 (장바구니·인증 UX 개선)

```text
현재 구현된 쇼핑몰 앱의 UX 및 장바구니 로직에서 아래 2가지 문제점을 수정해줘.
1. [UX 개선] '장바구니 담기' 클릭 시 토스트/버튼 텍스트 변경 등 피드백 추가
2. [로직 수정] 로그아웃 시 장바구니(cart) 상태 초기화
   - authSlice 또는 authListener에서 로그아웃 처리 시 cartSlice의
     resetCart(또는 clearCart) action을 dispatch
조건:
- 기존 Redux 구조와 Firebase auth listener 흐름을 유지할 것
- 변경된 파일과 테스트 방법을 안내할 것
```

### 대표 프롬프트 5 (헤더 아이콘화)

```text
상단 네비게이션 헤더 레이아웃 및 라우팅 구조를 아래 조건에 맞게 수정해줘.
1. 좌측: 'ShopMall' 로고만 남기고 텍스트 링크 제거
2. 우측: 장바구니 아이콘 + 프로필/로그인 아이콘 순서로 배치
3. 로그인 상태별 프로필 UI: 아바타/아이콘 + 드롭다운(이름+로그아웃)
4. 랜딩 페이지/라우팅 구조 정리
조건:
- 아이콘 라이브러리(lucide-react 등) 활용
- 변경 파일과 수정 코드를 기능 단위로 제시
```
...
|    9 | 담기 클릭 피드백                        | 토스트 노출 + 버튼이 "담겼습니다!"로 잠시 전환 + 헤더 배지 증가       | 코드 확인됨(수동 클릭 테스트 필요)          |  ☐   |
|   10 | 로그인 후 담기 → 로그아웃               | 로그아웃 즉시 장바구니가 비워지고 배지도 사라짐                       | 코드 확인됨(수동 테스트 필요)               |  ☐   |
|   11 | 게스트로 담기 → 새로고침 (로그인 안 함) | 장바구니가 그대로 유지됨(로그아웃 전환이 아니므로 `resetCart` 미발동) | 코드 확인됨(회귀 테스트 필요)               |  ☐   |
...
| 실제 이메일·UID·주소·전화번호가 캡처에 없습니다.      | ☐ (아직 스크린샷 없음 — 이제 헤더에 Google 프로필 사진/이메일도 표시되므로, 캡처 시 본인 계정 정보와 얼굴 사진이 보이지 않게 확인 필요) |
...
1. 저장할 원본 상태(cart의 `productId`/`quantity`)와 화면에서 파생시켜 계산할 값(총액, 상품 이름·가격 조인)을 구현 전에 먼저 구분해두니, 나중에 요구사항이 바뀌어도(로그아웃 시 초기화, 헤더 배지 추가 등) 리듀서 자체는 거의 안 건드리고 통합 지점만 수정하면 됐다.
2. Redux Toolkit의 `createSlice`는 Immer를 통해 `state.items.push(...)`처럼 직접 mutation하는 것처럼 보이는 코드를 안전하게 불변 업데이트로 바꿔준다는 걸 실제로 확인했다. 반면 일반 컴포넌트 상태(`useState`)에서 같은 패턴을 쓰면 진짜 버그가 된다는 차이를 명확히 인지하게 됐다.
3. 데이터·상태·인증을 먼저 안정화하고 UI/스타일링을 가장 나중에 입히니, 화면 마크업이 여러 번 바뀌어도(카드형 그리드, 아이콘 헤더 등) Redux 로직은 한 번도 다시 설계할 필요가 없었다.
...
- 미완료 기능: 상품 상세 페이지(`ProductDetailPage`)가 placeholder 상태, 검색/카테고리 필터/LocalStorage 미구현, 인증 로딩 UX·재로그인 시 에러 잔존·로그아웃 실패 처리(경미, 보류) 3건, 프로필 드롭다운 키보드(Esc) 대응 없음
...
| 프로필 드롭다운을 Esc 키로 닫을 수 없음 | 바깥 클릭 감지만 구현, 키보드 핸들러 없음 | `keydown` 리스너로 Esc 처리 추가                                 | 낮음     |
...
| 실행·배포 URL   | 아직 배포하지 않음 — 로컬 실행은 `npm install && npm run dev` |
